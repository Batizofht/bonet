      ...
      <div className="max-w-6xl mx-auto px-4 py-20">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mt-3 mb-4">
            {t("clientReviews.title").split(" ").slice(0, 2).join(" ")} <span className="text-[#C9A84C]">{t("clientReviews.title").split(" ").slice(2).join(" ")}</span>
          </h2>
          <p className="text-gray-500 text-lg max-w-2xl mx-auto">{t("clientReviews.subtitle")}</p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-center justify-center mb-12">
          ...
          <div
            key={selected.id}
            className="bg-white rounded-3xl p-8 border border-gray-200/30 lg:w-2/3"
          >
            <div className="flex items-start gap-4 mb-6">
              <div className="w-16 h-16 bg-[#C9A84C] rounded-2xl flex items-center justify-center flex-shrink-0">
          ...
          >
            <div className="relative group">
              <div className="w-48 h-48 mx-auto rounded-3xl overflow-hidden border border-gray-200/40">
                <img 
                  src={selected.image} 
                  alt={selected.name} 
        ...
        <div className="relative">
          <button 
            onClick={handlePrev} 
            className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 w-12 h-12 bg-white border border-gray-200/40 rounded-full flex items-center justify-center text-[#C9A84C] hover:bg-[#C9A84C] hover:text-white transition-all duration-300 z-10"
          >
              ...
              <div
                ...
                className={`flex flex-col items-center cursor-pointer p-4 rounded-2xl transition-all duration-300 min-w-[120px] ${
                  selected.id === person.id 
                    ? 'bg-[#C9A84C]' 
                    : 'bg-white border border-gray-200/30 hover:border-[#C9A84C]/40'
                }`}
          ...
          <button 
            onClick={handleNext} 
            className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 w-12 h-12 bg-white border border-gray-200/40 rounded-full flex items-center justify-center text-[#C9A84C] hover:bg-[#C9A84C] hover:text-white transition-all duration-300 z-10"
          >
          ...
          {Array.from({ length: Math.ceil(testimonials.length / 3) }).map((_, i) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${
                index === i * 3 ? 'bg-[#C9A84C] w-8' : 'bg-gray-300'
              }`}
            />
          ))}
        </div>
